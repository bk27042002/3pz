...
from matplotlib import patches as pchs
...
class Detection():
    ...
    def function(self, x, y, tp):
        if tp == 'bright_hist':
            ...
            # 256 - количество ячеек, [0, 256] - диапазон
    ...
    def get_majors(self, results):
        ...
        for i in range(self.m):
            for j in range(self.t):
                ...
                for k in range(5):
                    voting.append(results[k][i][j][1])

                majors[i].append(self.find_major(voting))
    ...
    def show_results(self):
        ...
        ax8 = plt.axes([0.1, 0.15, 0.2, 0.06])
        button = btn(ax8, 'Остановить', color='gray')
        ...
        for i in range(self.m):
            for j in range(self.t):
                ...
                ax7.cla()
                ax7.imshow(Image(majors[i][j], 0,
                                 'image').get_image())

                ax7.set_xticks([])
                ax7.set_yticks([])
                ax7.set_title('Голосование')
                ax7.set_xlabel('Класс = ' + str(majors[i][j] + 1))
    ...
    def show_histograms(self):
        ...
        for i in range(self.m):
            for j in range(self.t):
                ...
                accuracy[5].append((trues / alls) * 100)
